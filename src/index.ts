...
app.post("/payment-sheet", async (_, res) => {
  ...
  // Fetch your Stripe account details
  const account = await stripe.accounts.retrieve("acct_1QpJKfD7DB0hX7zl");

  // Check if custom application fee settings exist
  let feePercentage = 10; // Default to 10% if not found
  let applicationFeePercent;
  try {
    applicationFeePercent = await stripe.applicationFees.list({ limit: 20 });
  } catch (error) {
    console.error("Error fetching application fee percentage:", error);
    return res
      .status(500)
  ...
  if (applicationFeePercent.data.length > 0) {
    applicationFeePercent.data.forEach((fee) => {
      console.log(`Application fee percentage for account:`, {
        ...
        calculated_application_fee_amount:
          fee.calculated_application_fee_amount,
        account: fee.account,
      });
    });
  }

  // console.log(`Application fee percentage for account: ${feePercentage}%`);
  ...
  // ) {
  //   feePercentage = account.settings.payments.application_fee_percent;
  // }
  ...
  const totalAmount = 99999; // Example amount in cents (50.99 EUR)
...
app.post("/create-checkout-session", async (req, res) => {
  ...
  const { port }: { port?: string } = req.body;
